...
/*************************************************************************************************************************
 *
 * @brief      This case show how to use COMP to compare the three phase signal with Common-node sensed signal.
 *
 *
 *************************************************************************************************************************/
...
int main(void)
{
    ...
    */
    ...
    /* Select U, V, W signal to  compare with Common-node sensed signal*/
    PHCOMP_SelectChannel(PHCOMP_U, PHCOMP_FROM_PHU);
    PHCOMP_SelectChannel(PHCOMP_V, PHCOMP_FROM_PHV);
    PHCOMP_SelectChannel(PHCOMP_W, PHCOMP_FROM_PHW);
    PHCOMP_Enable(PHCOMP_U);
    PHCOMP_Enable(PHCOMP_V);
    PHCOMP_Enable(PHCOMP_W);
    PHCOMP_SelectReference(PHCOMP_REF_PHC);
    printf("PHCOMPCTL is %x\n", PHCOMP->PHCOMPCTL);

    /* Enable PHCOMP trigger INT in MCU side */
    NVIC_EnableIRQ(PHCOMP_IRQn);

    /* Enable U, V, W phase INT */
    PHCOMP_EnableInt(PHCOMP_INT_PHU_ZERO_CROSS | PHCOMP_INT_PHV_ZERO_CROSS | PHCOMP_INT_PHW_ZERO_CROSS);
...
void PHCOMP_IRQHandler(void)
{
    ...
    /* Clear U, V, W phase INT */
    PHCOMP_ClearInt(PHCOMP_INT_PHU_ZERO_CROSS | PHCOMP_INT_PHV_ZERO_CROSS | PHCOMP_INT_PHW_ZERO_CROSS);

    /* Clear global INT */
    PHCOMP_ClearInt(PHCOMP_INT_GLOBAL);
}
