...
void Master_Bulk_TxRX_data(I2C_REGS* I2Cx)
{
    ...
    if (MASTER_TRANSMIT)
    {
        ...
        I2C_MasterBulkWrite(I2Cx, I2C_ADDR_7BIT, I2C_Slave_ADDR, gau8TxBuf, gu32BuffSize);
    }
    else
    {
        ...
        I2C_MasterBulkRead(I2Cx, I2C_ADDR_7BIT, I2C_Slave_ADDR, gau8RxBuf, gu32BuffSize);
    ...
    /*
    * In the interface of 'I2C_MasterBulkWriteData()', master will wait for the Tx FIFO empty
    * after this bulk, in other words, the master will sent a 'STOP' CMD to the IIC, and
    * then sent a 'RESTART' at the next bulk. we can count the bulk we has sent to get
...
/*************************************************************************************************************************
 *
 * @brief      TIn this case, the IIC controller is used as master and enable the 'STOP' INT, and sent datum to slave with
 *             a 'STOP' CMD after every bulk, so the number of INT equal the number of bulk had sent.
 *
 *             This demo code is couple with the slave demo code named "I2C_Slave_Bulk_Polling_TxRx".
 *
 *************************************************************************************************************************/
...
int main(void)
{
    ...
    */
    ...
    if(estatus == ERROR)
    {
        printf("[IIC master initial FAIL] I2C clock is not fast enough to support the speed\n");
        return 0;
    }
