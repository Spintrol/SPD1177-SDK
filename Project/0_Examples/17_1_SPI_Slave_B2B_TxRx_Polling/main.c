...
/*************************************************************************************************************************
 *
 * @brief      In this case, Slave sent the datum received from master back to master with Back-to-Back mode. There two ways
 *             to trigger Back-to-Back mode:
 *                    1).TX FIFO is not empty while 'SFRAM' signal is valid period.
 *                    2).If the data width is less then 16bits, and when the Back-to-Back mode is enable. what's more, there
 *                       is a key point : The max speed of Back-to-Back mode is 4M, because of the limit of MCU can cause the
 *                       TX FIFO empty to break out the Back-to-Back mode.
 *
 *             This demo code couple with another named 'SPI_Master_B2B_TxRx_Polling'.
