...
void Master_TxRX_data(I2C_REGS* I2Cx)
{
    ...
    if (MASTER_TRANSMIT)
    {
        ...
        I2C_MasterWrite(I2Cx, I2C_ADDR_7BIT, I2C_Slave_ADDR, gau8TxBuf, gu32BuffSize);
    }
    else
    {
        ...
        I2C_MasterRead(I2Cx, I2C_ADDR_7BIT, I2C_Slave_ADDR, gau8RxBuf, gu32BuffSize);
    ...
    /*
    * Check interrupt counter, if the INT count is not equal the bytes we has sent, there
...
/*************************************************************************************************************************
 *
 * @brief      In this case, the IIC controller is used as master enabled the 'STOP' INT, and sent data to slave with a
 *             'STOP' CMD after every byte, so the number of INT equal the number of bytes had sent.
 *
 *             This demo code is couple with the slave demo code named "I2C_Slave_Polling_TxRx".
 *
 *************************************************************************************************************************/
