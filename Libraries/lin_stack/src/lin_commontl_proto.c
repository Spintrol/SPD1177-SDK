...
#if (_TL_FRAME_SUPPORT_ ==  _TL_MULTI_FRAME_)
void lin_tl_make_multi_slaveres_pdu
(
    /* [IN] service identifier */
    l_u8 sid
)
{
    ...
    l_u8 recdata[100];
    l_u8 senddata[10] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xaa};

    /* get pdu from rx queue */
    ld_receive_message(&length, recdata);

    
    if (sid == SERVICE_READ_DATA_BY_IDENTIFY)
    {
        // Set response length and senddata according to recdata
        senddata[0] = sid + 0x40;
        length = 10;
    }
    else if (sid == SERVICE_WRITE_DATA_BY_IDENTIFY)
    {
        // Set response length and senddata according to recdata
        senddata[0] = sid + 0x40;
        length = 10;
    ...
    ld_send_message(length, senddata);
}
#else
    ...
    if (sid_supported_flag == 1)
    {
        ...
        switch (sid)
        {
            ...
            case SERVICE_READ_DATA_BY_IDENTIFY:
                /* Response to master - RSID */
                lin_tl_make_multi_slaveres_pdu(SERVICE_READ_DATA_BY_IDENTIFY);
                break;
            case SERVICE_WRITE_DATA_BY_IDENTIFY:
                /* Response to master - RSID */
                lin_tl_make_multi_slaveres_pdu(SERVICE_WRITE_DATA_BY_IDENTIFY);
                break;
            #else
